...
func TestUserCp(t *testing.T) {
	...
	fmt.Println("对象池", GetManage().UserManage.users[100001], "上下文中的原始数据", updateUserMap.(*UpdateUsersStruct).userWatcher[100001].originalUser, "上下文中的当前数据", updateUserMap.(*UpdateUsersStruct).userWatcher[100001].currentUser, "二次获得", u2)
}
