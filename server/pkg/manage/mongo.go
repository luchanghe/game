	...
	mongoManageOnce.Do(func() {
		poolMonitor := &event.PoolMonitor{
			Event: func(evt *event.PoolEvent) {
				log.Printf("connection pool event: %s\n", evt)
			},
		}
		...
		option = option.SetPoolMonitor(poolMonitor)
		option = option.SetMinPoolSize(GetConfigManage().GetUint64("mongo.min_pool_size"))
		option = option.SetMaxConnIdleTime(time.Second * time.Duration(GetConfigManage().GetUint64("mongo.max_conn_minute")))
		var err error
